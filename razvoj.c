...
int main()
{
    ...
    { 
      ...
      if(taster == '2'){
        crta[pozicija]++;
        if(crta[pozicija] > 3) crta[pozicija] = 3;
        lcd.write(byte(crta[pozicija]));
        taster = '0';
         _delay_ms(500);
      }
      ...
      if(taster == '8'){
        crta[pozicija]--;
        if(crta[pozicija] < 0) crta[pozicija] = 0;
        lcd.write(byte(crta[pozicija]));
        taster = '0';
         _delay_ms(500);
      }
      ...
      if(taster == '5'){
        ...
        bool ok = false;
        for(int i = 0; i < 4; i++){
          if(crta[i] == kombinacija[i]){
            ok = true;
          }
        }
        
        if(ok == true) lcd.print("Dobro je");
        else lcd.print("Netacno!");
      }
